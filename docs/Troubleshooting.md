# Troubleshooting
This page contains documentation for common issues and known incompatibilities, as well as a guide to help you create a helpful bug report.

Before reporting a bug, please read through the following sections to see if your issue has already been addressed.
...
- Ensure you are running a 64-bit application (try `PROTON_USE_WOW64=1`, but if it still doesn't work you're out of luck).
...
If applications do not open at all with lsfg-vk enabled (stuck at black screen):
- Ensure you configured the correct `gpu` for this profile, in case you have multiple GPUs and/or GPU drivers (lsfg-vk-ui will show all available GPUs in a dropdown), lsfg-vk might be defaulting to a different one than the game is using
...
When opening a bug report, please include the following information to help us diagnose and fix your issue:
- A detailed description of the issue you are experiencing.
- Details about the system you are running on (OS, GPU, relevant drivers, etc.).
- The game or application you are trying to run (and through what platform: e.g., Steam Proton, native Linux, etc.).
...
If you're running the game through Steam, this log file is located at `~/.steam/steam/logs/console-linux.txt`. Please clear it before launching the game to ensure it only contains relevant information.